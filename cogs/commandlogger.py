...
class CommandLogger(commands.Cog):
    ...
    async def on_command_error(self, ctx, error):
        if isinstance(error, commands.CommandNotFound):
            omega.logger.info(f'Command not found: {ctx.message.content}')
        elif isinstance(error, commands.MissingRequiredArgument):
            await ctx.send('Missing required argument.')
            omega.logger.info(f'Missing required argument in command: {ctx.message.content}')
        else:
            await ctx.send(f'{str(error)}')
            omega.logger.error(f'An error occurred: {str(error)}')
            raise error
    ...
    async def on_command(self, ctx):
        ...
        omega.logger.info(f"Command '{command_content}' entered by {user_info} in {server_name} ({channel_name}) [channel id: {channel_id}]")
